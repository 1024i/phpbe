...
class article extends \admin\system\controller
{
	...
	public function articles()
	{
		...
		$pagination->set_total($admin_model_article->get_article_count($option));
		...
		$articles = $admin_model_article->get_articles($option);
		foreach ($articles as $article) {
			$article->comment_count = $admin_model_article->get_comment_count(array('article_id'=>$article->id));
		}
    ...
    public function edit()
	{
		...
		$template = be::get_admin_template('article.edit');
		if ($id == 0) {
	...
	public function edit_save()
	{
		...
		if ($row_article->save()) {
			if ($id == 0) {
				$this->set_message('添加文章成功！');
				system_log('添加文章：#'.$row_article->id.': '.$row_article->title);
			} else {
				$this->set_message('修改文章成功！');
				system_log('编辑文章：#'.$id.': '.$row_article->title);
			}
		} else {
			$this->set_message($row_article->get_error(), 'error');
		}
    ...
    public function save_categories()
    {
		...
		$this->set_message('保存分类成功！');
        $this->redirect('./?controller=article&task=categories');
    }
	...
	public function comments()
	{
		...
		$pagination = be::get_admin_ui('pagination');
		$pagination->set_limit($limit);
		$pagination->set_total($admin_model_article->get_comment_count($option));
		$pagination->set_page(request::post('page', 1, 'int'));
		...
		$template->set('article_id', $article_id);
		...
		$articles = array();
		$comments = $admin_model_article->get_comments($option);
	...
	public function setting()
	{
		...
		$template->set_title('设置文章系统参数');
		$template->set('config_article', be::get_config('article'));
		$template->display();
	}
	...
	public function setting_save()
	{
		...
		$this->set_message('设置成功！');
		$this->redirect('./?controller=article&task=setting');
	}
