...
class system_filemanager extends \admin\system\controller
{
	...
	public function browser()
	{
		...
		$template->set('sort', $sort);
		$template->set('filter_image', $filter_image);
		$template->set('src_id', $src_id);
	...
	public function upload_file()
	{
		...
		if ($file['error'] == 0) {
			...
			if (!in_array($type, $config_system->allow_upload_file_types)) {
				$this->set_message('不允许上传('.$type.')格式的文件！', 'error');
				$this->redirect($return);
			}
			
			if (strpos($file_name, '/') !== false) {
				$this->set_message('文件名称不合法！', 'error');
				$this->redirect($return);
			}
			...
			if (move_uploaded_file($file['tmp_name'], $dst_path)) {
				...
				if ($rename == false) {
					$this->set_message('上传文件成功！');
				} else {
					$this->set_message('有同名文件，新上传的文件已更名为：'.$rename.'！','warning');
				}
			} else {
				$this->set_message('上传失败！', 'error');
			}
		} else {
