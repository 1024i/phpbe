...
class user extends \system\model
{
	...
	public function login($username, $password, $remember_me = false)
	{
		...
		$user = db::get_object('SELECT * FROM `be_user` WHERE `username`=\''.$username.'\'');
		...
		if ($user) {
			if ($user->password == $password) {
				...
				session::delete($ip);
				session::set('_user', be::get_user($user->id));

				db::execute('UPDATE `be_user` SET `last_login_time`='.time().' WHERE `id`='.$user->id);
				if ($remember_me) {
					$config_user = be::get_config('user');
					$remember_me = $username.'|||'.$this->encrypt_password($user->password);
		...
		if (cookie::has('_remember_me')) {
			...
			if ($remember_me) {
				...
				if (count($remember_me) == 2) {
					...
					$user = db::get_object('SELECT * FROM `be_user` WHERE `username`=\''.db::escape($username).'\'');
	...
	{
		if (db::get_result('SELECT COUNT(*) FROM `be_user` WHERE `username`=\''.$username.'\'')) {
			$this->set_error('用户名('.$username.')已被占用！');
			return false;
		}

		if (db::get_result('SELECT COUNT(*) FROM `be_user` WHERE `email`=\''.$email.'\'')) {
		...
		if ($config_user->email_valid == '1') {
            ...
            $subject = $this->format($config_user->register_mail_activation_subject, $datas);
            $body = $this->format($config_user->register_mail_activation_body, $datas);
			...
			if ($config_user->email_register == '1') {
                ...
                $subject = $this->format($config_user->register_mail_subject, $datas);
                $body = $this->format($config_user->register_mail_body, $datas);
        ...
        if ($config_user->email_register_admin!='') {
            if ($this->is_email($config_user->email_register_admin)) {
                ...
                $subject = $this->format($config_user->register_mail_to_admin_subject, $datas);
                $body = $this->format($config_user->register_mail_to_admin_body, $datas);
	...
	public function forgot_password($username)
	{
        ...
        $subject = $this->format($config_user->forgot_password_mail_subject, $datas);
        $body = $this->format($config_user->forgot_password_mail_body, $datas);
	...
	public function forgot_password_reset($user_id, $token, $password)
	{
        ...
        $subject = $lib_mail->format($config_user->forgot_password_reset_mail_subject, $datas);
        $body = $lib_mail->format($config_user->forgot_password_reset_mail_body, $datas);
