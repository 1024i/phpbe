...
foreach ($files as $file) {
    if ($file['type']!='dir') {
        ...
        <li>
            <div class="file">
                <a href="javascript:;" onclick="javascript:<?php echo $filterImage == 1?'selectImage':'selectFile'; ?>('<?php echo $file['name']; ?>', '<?php echo URL_ROOT.'/'.DATA.$path.'/'.$file['name']; ?>', '<?php echo $srcId ?>');">
                    <?php
